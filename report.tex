...
We use four publicly available datasets for our exploratory analysis. We combine datasets \cite{waseem2016you}, \cite{rizoiu2019transfer}, and \cite{jha2017does} into one large dataset (reffered to as Dataset SRB) as they include same categories of hate speech. We make labels \textit{sexism}, \textit{racism}, and \textit{both} from \cite{waseem2016you} and \cite{rizoiu2019transfer}; regarding the third dataset (\cite{jha2017does}) we tranform labels \textit{hostile sexism} into \textit{sexism}, as tweets with such labels are already included in the first two datasets, and \textit{benevolent sexism} into \textit{benevolent}. We exclude \textit{None} label from all datasets as we do not need it for the analysis.

The fourth dataset (reffered to as Dataset AHS) that we use \cite{founta2018large} has 4 additional categories \textit{abusive,
hateful, spam, and normal}, where we again exclude the latter category, as we are not interested in tweets that do not include hate speech. 

We show the distribution of individual categories from datasets SRB and AHS in Figures \ref{fig:distribution_tweets_dataset1} and \ref{fig:distribution_tweets_dataset2}, respectively.

\begin{figure}[ht]\centering
	\includegraphics[width=\linewidth]{distribution_tweets_dataset1.png}
	\caption{This figure shows the distribution of hate speech categories in the first dataset. We can see that sexism and benevolent are well represented, whereas racism and both are far less frequent.}
